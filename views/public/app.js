(function() {
	...
	app.config(function($routeProvider, $locationProvider, authProvider, $httpProvider, jwtInterceptorProvider) {
		...
		when('/weather', {
			...
			title: 'Weather2 - Buffalo Projects',
			isWeather : 'true'
		}).
		when('/weatherterms', {
			templateUrl: '/views/public/templates/weatherterms.html',
			controller: 'WeatherTermsController',
			title: 'Weather1 - Buffalo Projects',
			isWeather : 'false'
		}).
		otherwise({
			redirectTo: '/home'
	...
	app.controller("WeatherTermsController", function($scope) {
		...
		$('#header').css({backgroundColor:'rgba(0,0,0,1.0)'});
	...
	app.controller('WeatherController', function($scope, $cookies, $upload, leafletData, auth, store, $http) {
		...
		$scope.searchForAddress = function() {
			$http({method:'GET', url:'http://open.mapquestapi.com/geocoding/v1/address', params: {key:'', location:$scope.currentPictureLocationLabel}}).
		...
		$scope.uploadWeatherImage =function($files) {
			...
			var bucket = new AWS.S3({params: {Bucket: 'buffaloimages'}});
			...
			if($scope.file) {
				...
				if(fileExtension ==".jpg" || fileExtension ==".jpeg" || fileExtension ==".png") {
					...
					var params = { Key: objKey, ContentType: $scope.file.type, Body: $scope.file  };
